...
Write-Host "`nTesting Search API`n" -ForegroundColor Cyan

$BASE_URL = "http://localhost:3000"
$API_KEY = $env:API_KEY

if (-not $API_KEY) {
    Write-Host "API_KEY not found in environment variables" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "   [OK] Fresh scrape successful" -ForegroundColor Green
...
try {
    ...
    Write-Host "   [OK] Cached search successful" -ForegroundColor Green
