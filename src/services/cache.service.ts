...
export class CacheService {
   ...
   */
  async cacheSearchResults(query: string, count: number, results: any): Promise<void> {
    const key = `search:${query}:${count}`;
    await this.set(key, results, 3600); // 1 hour
    logger.debug(`Cached search results for: ${query} (${count} items)`);
  }

  /**
   * Get cached search results
   */
  async getCachedSearchResults(query: string, count: number): Promise<any | null> {
    const key = `search:${query}:${count}`;
   ...
   */
  async cacheVehicle(lotNumber: string, data: any): Promise<void> {
   ...
   */
  async getCachedVehicle(lotNumber: string): Promise<any | null> {
    const key = `vehicle:${lotNumber}`;
    const cached = await this.get(key);
    if (cached) {
