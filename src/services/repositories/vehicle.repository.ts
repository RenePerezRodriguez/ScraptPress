...
export class VehicleRepository {
   ...
   */
  static async upsertVehicle(vehicle: VehicleData | any): Promise<boolean> {
    try {
      ...
      const model = (vehicle as any).model || (vehicle as VehicleData).vehicle_model;

      // Debug logging
      logger.debug(`Upserting vehicle: lot=${vehicle.lot_number}, make=${vehicle.make}, model=${model}, year=${vehicle.year}`);

      // Generate search tokens for efficient searching
      // Include individual words from make and model for better matching
      const makeWords = vehicle.make?.toLowerCase().split(/\s+/).filter(Boolean) || [];
      ...
      const search_tokens = [
        ...
        `${vehicle.make} ${model}`.toLowerCase(),
        `${vehicle.year} ${vehicle.make} ${model}`.toLowerCase(),
      ...
      logger.debug(`Vehicle ${vehicle.lot_number} saved with ${search_tokens.length} search tokens`);
      return true;
    } catch (error) {
  ...
  static async searchVehicles(
    ...
    try {
      ...
      let firestoreQuery = db.collection(COLLECTIONS.VEHICLES)
      ...
      if (snapshot.size === 0) {
        logger.warn(`No documents found. Query: tokens=${JSON.stringify(tokens)}, limit=${limit}, offset=${offset}`);
      }
  ...
  static async getPopularSearches(limit: number = 10): Promise<any[]> {
    try {
      ...
      const searchCounts = new Map<string, { query: string; count: number; avg_results: number; total_results: number }>();
      ...
      snapshot.forEach(doc => {
        ...
        if (query) {
          const existing = searchCounts.get(query) || { query, count: 0, avg_results: 0, total_results: 0 };
          existing.count++;
   ...
   */
  static async getApiHealthMetrics(): Promise<any[]> {
    try {
      const db = getFirestore();
      const oneDayAgo = new Date();
      oneDayAgo.setHours(oneDayAgo.getHours() - 24);

      const snapshot = await db.collection(COLLECTIONS.API_REQUESTS)
